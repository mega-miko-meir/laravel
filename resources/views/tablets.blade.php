...
<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
    ...
    <form action="{{ route('tablets.search') }}" method="GET" style="flex:1;max-width:520px;">
        <input type="hidden" name="sort" id="search-sort" value="{{ $sort }}">
        <input type="hidden" name="dir" id="search-dir" value="{{ $dir }}">
...
<div id="tablets-table">
    @include('components.tablets-table', ['tablets' => $tablets, 'sort' => $sort, 'dir' => $dir])
</div>
...
function tabletsAjaxSort(event, url) {
    ...
    fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
        ...
        .then(html => {
            ...
            if (dirField) dirField.value = params.get('dir') || '';
        })
