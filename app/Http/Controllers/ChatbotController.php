...
ВАЖНО про формат финального ответа: если результат — это список из НЕСКОЛЬКИХ записей
с несколькими полями каждая (сотрудники, SKU, дистрибьюторы, продажи и т.п.) — оформляй
его ТОЛЬКО как Markdown-таблицу (шапка `| Колонка | Колонка |`, разделитель `|---|---|`,
дальше строки данных), а не нумерованным списком — так пользователь сможет скопировать
таблицу и вставить в Excel. Для одиночного числа/факта — обычный текст без таблицы.
...
обратных кавычках (backtick), напр. `Медпредставитель`, `Amount_disc`.
  ...
  `Дост_колво` — доставленное количество. ВАЖНО: для любых вопросов про "количество"/
  "сколько продано" всегда используй именно эту колонку (SUM(`Дост_колво`)), а не
  `Заказ_упаковки` (это количество в заказе ДО доставки, не фактические продажи)
  `Брэнд` — бренд препарата
  `SKU_splitted` — полное название SKU (препарат/форма/дозировка/фасовка одной строкой)
  `Distributor` — дистрибьютор (короткое название, напр. "Стофарм")
  `Distributor_branch` — дистрибьютор с указанием филиала/города (напр. "Стофарм (Караганда)")
  `Название аптеки`, `Город аптеки`, `ID аптеки` — аптека
  `Город` — город КМП
  `Department` — департамент, значения ровно 'OTC' или 'Rx' (используй как есть,
  не путать с `Бизнес-подразделение` — это другая, более дробная группировка команды)
  ...
  Вопрос: Какие SKU были проданы?
  DATA_QUERY: SELECT `SKU_splitted`, SUM(`Дост_колво`) AS qty, SUM(`Amount_disc`) AS total_sales FROM kmp WHERE `Статус заказа` = 'Доставлено' GROUP BY `SKU_splitted` ORDER BY total_sales DESC LIMIT 20

  Вопрос: Выведи лидеров по дистрибьютерам по продажам КМП за 2026
  DATA_QUERY: SELECT `Distributor`, SUM(`Amount_disc`) AS total_sales FROM kmp WHERE `Год` = 2026 AND `Статус заказа` = 'Доставлено' GROUP BY `Distributor` ORDER BY total_sales DESC LIMIT 20

Таблица: IMS_YTD_2026_for_PowerBI — данные IMS по ВСЕМУ рынку Казахстана (все конкуренты,
не только Nobel), 641 тыс. строк. ВАЖНО: названия колонок содержат пробелы/слэши/$ —
ВСЕГДА оборачивай в обратные кавычки, напр. `` `YTD/2026 $` ``, `` `RX/OTC` ``.

  `SKU` — полное название SKU (продукт/форма/дозировка/фасовка)
  `Umbrella Brand` — название бренда
  `Corporation`, `Manufacturer` — компания-производитель. Для "продажи Nobel" фильтруй
  `Corporation` LIKE '%NOBEL%' (в данных встречается как 'NOBEL-AFF', 'NOBEL ILAC SAN TUR')
  `Country` — страна ПРОИЗВОДИТЕЛЯ (напр. SWITZERLAND у Roche), а НЕ страна продажи.
  ВСЯ таблица — это рынок Казахстана (Region/Territory всегда казахстанские), просто
  препараты бывают импортные. НИКОГДА не фильтруй по Country='KAZAKHSTAN', чтобы получить
  "рынок Казахстана" — так отфильтруются только отечественные препараты и общий рынок
  будет занижен. Используй всю таблицу без фильтра по Country для вопросов про весь рынок.
  `Region`, `Territory` — регион/город продажи в Казахстане
  `Channel` — 'RETAIL' или 'BUDGET'; `segment` — детальнее (Hospital, Outpatient reimbursement, RETAIL)
  `RX/OTC` — значения НЕ 'RX'/'OTC' буквально, а: 'PRESC' = рецептурный (Rx),
  'NPRES' = безрецептурный (OTC), 'NAPPL' = не применимо
  Колонки продаж по годам — паттерн `{year} {suffix}` для завершённых лет и
  `YTD/{year} {suffix}` для текущего/неполного года (year-to-date, не сравнивай напрямую
  с полным годом других лет без поправки). suffix: `$` — сумма в USD, `U` — количество
  единиц, `KZ` — сумма в тенге, `Price $`/`Price Tg` — средняя цена за единицу.
  Актуальные: `` `YTD/2026 $` ``, `` `YTD/2026 U` ``, `` `YTD/2026 KZ` ``.

  Вопрос: Какая доля рынка Nobel в Казахстане за 2026 год (YTD)?
  DATA_QUERY: SELECT SUM(`YTD/2026 KZ`) AS total_market, SUM(CASE WHEN Corporation LIKE '%NOBEL%' THEN `YTD/2026 KZ` ELSE 0 END) AS nobel_sales FROM IMS_YTD_2026_for_PowerBI
...
PROMPT;
    ...
    private function executeAndAnswer(string $question, string $sql, array $messages): string
    {
        ...
        // Таблицу строим детерминированно в PHP из реальных строк, а не просим модель её
        // отформатировать — модель не всегда следует инструкции "оформи как таблицу"
        // (особенно при длинной истории диалога), из-за чего кнопка копирования в чате
        // просто не появлялась. Так таблица гарантированно корректна и всегда на месте.
        $table = $this->toMarkdownTable($rows);

        // Шаг 2: Gemini формулирует ответ на основе реальных данных
        $instruction = $table !== null
            ? 'Данные уже получены — это финальный шаг. Полная таблица со всеми записями будет показана '
                . 'ОТДЕЛЬНО после твоего ответа — НЕ перечисляй записи по одной, дай только краткий вывод '
                . '(1-2 предложения: сколько записей, главный факт/сумма). НЕ используй markdown-таблицы и списки.'
            : 'Данные уже получены — это финальный шаг. Дай чёткий понятный ответ на русском языке обычным текстом.';

        $messages[] = [
            'role'    => 'user',
            'content' => "Вопрос: {$question}\n\nДанные из БД:\n{$data}\n\n{$instruction} "
                . 'НЕ используй маркеры DATA_QUERY/DATA_QUERY_MYSQL/DATA_QUERY_NOBEL и не пиши SQL — они здесь неуместны.',
        ];

        $prose = trim($this->callGemini($messages));

        return $table !== null ? "{$prose}\n\n{$table}" : $prose;
    }

    /**
     * Строит Markdown-таблицу из строк результата SQL-запроса. Возвращает null,
     * если строить таблицу не имеет смысла (меньше 2 строк или меньше 2 колонок —
     * это одиночное число/факт, а не список записей).
     */
    private function toMarkdownTable(array $rows, int $maxRows = 30): ?string
    {
        if (count($rows) < 2) {
            return null;
        }
        ...
        foreach ($rows as $row) {
            $lines[] = '| ' . implode(' | ', array_map($escape, array_values((array) $row))) . ' |';
        }
    ...
    private function executeMultiAndAnswer(string $question, string $step1, array $messages): string
    {
        ...
        $messages[] = [
            'role'    => 'user',
            'content' => "Вопрос: {$question}\n\nДанные из БД:\n{$data}\n\n{$instruction} Данные уже получены — это финальный шаг. "
                . 'Если в ответе больше одной записи — оформи ОБЯЗАТЕЛЬНО как Markdown-таблицу '
                . '(`| Колонка | Колонка |`, разделитель `|---|---|`, затем строки), это критично для пользователя '
                . '(копирует в Excel). Для одиночного числа/факта — обычный текст. '
                . 'НЕ используй маркеры DATA_QUERY/DATA_QUERY_MYSQL/DATA_QUERY_NOBEL и не пиши SQL — они здесь неуместны.',
        ];
    ...
    private function pickConnection(string $sql): string
    {
        ...
        $nobelTables = ['qs_calls', 'qs_onekey', 'stg_nobel', 'ims_ytd_2026_for_powerbi'];
